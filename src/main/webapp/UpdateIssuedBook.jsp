<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="model.IssueBook, dao.IssueBookDAO, daoimpl.IssueBookDAOImp"%>
...
<body>
    ...
    <h5 style="margin: 15px;">Issued Book Details</h5><hr>
	<%
		int issuedBookId = (int) session.getAttribute("issuedBookId");
		IssueBookDAO issueBookDAO = new IssueBookDAOImp();
		IssueBook issueBook = issueBookDAO.getIssueBookById(issuedBookId);
	%>
    <div class="container">
      <table class="table" border="1">
        <thead class="thead">
          <th>Issued Book Details</th>
        </thead>
        <tbody>
          <tr>
            <td>
              <form action="UpdateIssuedBook">
	              <label for="">Student Name: <%= issueBook.getUser().getName() %></label><br>
	              <label for="">Book Name: <%= issueBook.getBook().getName() %> </label><br>
	              <label for="">ISBN Number: <%= issueBook.getBook().getIsbnNumber() %></label><br>
	              <label for="">Book Issued Date: <%= issueBook.getIssueDate() %></label><br>
	              <label for="">Book Returned Date: <%= issueBook.getReturnDate() %></label><br>
	              <label for="">Fine (in USD)</label><br>
	              <input class="form-control" type="text" name="fine"><br>
